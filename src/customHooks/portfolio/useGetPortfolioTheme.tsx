...
export const useGetPortfolioTheme = (portfolio: Portfolio) => {
    const getPortfolioTheme = () => {
        switch (portfolio.themeId) {
            ...
            case ThemeEnum.DEFAULT_DARK:
                return <DefaultPortfolioTheme portfolio={portfolio} />;
            case ThemeEnum.FLOYKA:
            case ThemeEnum.FLOYKA_DARK:
                return <FloykaPortfolioTheme portfolio={portfolio} />;
            case ThemeEnum.ETOPES:
            case ThemeEnum.ETOPES_DARK:
                return <EtopesPortfolioTheme portfolio={portfolio} />;
            case ThemeEnum.ARTISAN:
            case ThemeEnum.ARTISAN_DARK:
                return <ArtisanPortfolioTheme portfolio={portfolio} />;
            case ThemeEnum.PERSONA:
                return <PersonaPortfolioPage portfolio={portfolio} />;
            default:
                return <DefaultPortfolioTheme portfolio={portfolio} />;
        }
    };
...
// NexTemp Tema - Mevcut projenizin teması
const NexTempPortfolioTheme = ({ portfolio }: { portfolio: Portfolio }) => {
    return (
        <>
            <Head>
...
// Diğer tema bileşenleri (placeholder)
const DefaultPortfolioTheme = ({ portfolio }: { portfolio: Portfolio }) => (
    <div>Default Theme - {portfolio.firstName} {portfolio.lastName}</div>
);

const FloykaPortfolioTheme = ({ portfolio }: { portfolio: Portfolio }) => (
    <div>Floyka Theme - {portfolio.firstName} {portfolio.lastName}</div>
);

const EtopesPortfolioTheme = ({ portfolio }: { portfolio: Portfolio }) => (
    <div>Etopes Theme - {portfolio.firstName} {portfolio.lastName}</div>
);

const ArtisanPortfolioTheme = ({ portfolio }: { portfolio: Portfolio }) => (
    <div>Artisan Theme - {portfolio.firstName} {portfolio.lastName}</div>
); 